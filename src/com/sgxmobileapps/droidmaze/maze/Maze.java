...
public class Maze {
	...
	public void addMarker(TextureRegion markerTexture, IEntity layer, PhysicsWorld physicsWorld) {
        ...
        Sprite markerSprite = new Sprite(0, 0, 
                markerTexture.getWidth() * scaleXY, 
                markerTexture.getHeight() * scaleXY, 
                markerTexture);
    ...
    //    pieceSprite.setScaleCenter(0, 0);
     //   pieceSprite.setScale(scaleXY, scaleXY);
        ...
        Body body = PhysicsFactory.createCircleBody(physicsWorld, markerSprite, BodyType.DynamicBody, markerFixtureDef);
        
        layer.attachChild(markerSprite);
        physicsWorld.registerPhysicsConnector(new PhysicsConnector(markerSprite, body, true, true));
	}
	
	public void drawMaze(int x, int y, int height, int width, IEntity layer, PhysicsWorld physicsWorld){
	    mPx = x;
	    mPy = y;
	    ...
	    /* box */
	    Shape ground = new Rectangle(x, y + (mStepY + WALL_WIDTH) * mHeight, 
	                                 WALL_WIDTH + (mStepX + WALL_WIDTH) * mWidth, WALL_WIDTH);
        Shape roof = new Rectangle(x, y, 
                                   WALL_WIDTH + (mStepX + WALL_WIDTH) * mWidth, WALL_WIDTH);
        Shape left = new Rectangle(x, y + WALL_WIDTH, 
                                   WALL_WIDTH, (mStepY + WALL_WIDTH) * mHeight - WALL_WIDTH);
        Shape right = new Rectangle((mStepX + WALL_WIDTH) * mWidth, y + WALL_WIDTH, 
                                    WALL_WIDTH, (mStepY + WALL_WIDTH) * mHeight - WALL_WIDTH);
            ...
            for (int j = 0; j < mWidth; j++){
                if (!mMazeGrid[i][j].isSouthWallClosed()) {
                    if (start >= 0) {
                        ...
                        Shape wallH = new Rectangle(x + ((start==0)?WALL_WIDTH:0) + start * (mStepX + WALL_WIDTH), 
                                                    y + (i+1) * (mStepY + WALL_WIDTH), 
                                                    (end - start) * (mStepX + WALL_WIDTH) + ((start==0)?0:WALL_WIDTH), 
                                                    WALL_WIDTH);
                        PhysicsFactory.createBoxBody(physicsWorld, wallH, BodyType.StaticBody, wallFixtureDef);
            ...
            for (int i = 0; i < mHeight; i++){
                if (!mMazeGrid[i][j].isEastWallClosed()) {
                    if (start >=0) {
                        ...
                        // TODO draw line from start to end
                        Shape wallV = new Rectangle(x + (j+1) * (mStepX + WALL_WIDTH), 
                                                    y + WALL_WIDTH + start * (mStepY + WALL_WIDTH), 
                                                    WALL_WIDTH, 
                                                    (end - start) * (mStepY + WALL_WIDTH));
                        PhysicsFactory.createBoxBody(physicsWorld, wallV, BodyType.StaticBody, wallFixtureDef);
